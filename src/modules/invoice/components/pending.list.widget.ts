...
@Component({
    ...
    template: `
        ...
        <div class="col-md-12 no-padding">
            <mat-toolbar>
                CUENTAS POR COBRAR
                <button class="action pull-right" style="position: absolute;right: 32px;" mat-button (click)="filter()" >
            ...
            <table class="table">
                ...
                <tbody>
                    <tr *ngFor="let invoice of visible_invoices">
                        ...
                        <td class="text-overflow" matTooltip="{{invoice.client.name}} {{invoice.client.last_name}}">{{invoice.client.name}} {{invoice.client.last_name}}</td>
                        <td>{{invoice.total | currency:'':'$':'1.2-2'}}</td>
                        <td>{{invoice.restant | currency:'':'$':'1.2-2'}}</td>
                        <td>
                            <button class="action" mat-button title="Agregar pago" (click)="create(invoice)">
...
export class PendingListWidget implements AfterViewInit {
    ...
    paginate(params: any) {
        ...
        this.visible_invoices = this.invoices.slice(current_size, current_size + params.pageSize)
    }
    ...
    load() {
        ...
        this.invoiceService.pending(this.current_filter).subscribe((response: any) => {
