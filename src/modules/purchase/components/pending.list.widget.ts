      ...
      <table class="table">
        ...
        <tbody>
          <tr *ngFor="let purchase of visible_purchases">
            ...
            <td
              class="text-overflow"
              matTooltip="{{ purchase.provider.name }} {{
                purchase.provider.last_name
              }}"
            >
              {{ purchase.provider.name }} {{ purchase.provider.last_name }}
            </td>
...
export class PurchasePendingListWidget implements AfterViewInit {
  ...
  paginate(params: any) {
    ...
    this.visible_purchases = this.purchases.slice(
      current_size,
    ...
    dialogRef.afterClosed().subscribe((result: any) => {
      if (result) {
        ...
        if (result.value >= (purchase as any)["restant"]) {
          ...
          requests.push(
            this.purchaseService.change_status(purchase._id, purchase)
          );
        }
