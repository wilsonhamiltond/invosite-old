...
export class UserService extends BaseService {
  ...
  public setUser(user?: IUser, token?: string) {
    if (user) {
      sessionStorage.setItem("invo_site_user", JSON.stringify(user));
    }
    if (token) {
      sessionStorage.setItem("invo_site_token", token);
    }
    this.user = user;
  }
  public logoff() {
  ...
  logged() {
    return this.request("post", `${this.base_url}/logged`, {});
  }
  login(user: IUser) {
    return this.request("post", `${this.base_url_v0}/login`, user);
  }
  passwordChange(user: IUser) {
    return this.request("post", `${this.base_url}/password`, user);
  }
  setting(name: string) {
    return this.request("get", `${this.base_url_v0}/${name}/setting`);
  }
  box(box: IBox) {
